...
export default function BookingDetailPage() {
  ...
  const handleApproveJob = async () => {
    ...
    setApproving(true);
    try {
      const updated = await updateBookingStatus(id, 'customer_approved');
      if (updated) {
        setBooking(updated);
  ...
  if (!booking) return (
    <div className="space-y-6 text-center py-12">
      <h2 className="font-hydro-display text-xl font-bold">Booking Not Found</h2>
      <p className="text-sm text-muted-foreground">The requested booking record could not be located.</p>
